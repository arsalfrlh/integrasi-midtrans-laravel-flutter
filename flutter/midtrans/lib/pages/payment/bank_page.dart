...
class _BankPageState extends State<BankPage> {
  ...
  late TextEditingController codePaymentController;
  ...
  final formated = DateFormat('dd MMM yyyy HH:mm');

  @override
  void initState() {
    super.initState();
    orderIDController = TextEditingController(text: widget.bank.orderID);
    codePaymentController = TextEditingController(text: widget.bank.codePayment);
    totalController = TextEditingController(text: widget.bank.total);
    transaksiController = TextEditingController(text: formated.format(widget.bank.transaksi));
    expiredController = TextEditingController(text: formated.format(widget.bank.expired));
    if(widget.bank.bank == 'bca'){
      bankController = TextEditingController(text: "Bank Central Asia");
    }else if(widget.bank.bank == 'bri'){
      bankController = TextEditingController(text: "Bank Rakyat Indonesia");
    }else if(widget.bank.bank == 'bni'){
      bankController = TextEditingController(text: "Bank Negara Indonesia");
    }else if(widget.bank.bank == 'cimb'){
      bankController = TextEditingController(text: "CIMB Niaga");
    }
  }
              ...
              child: Column(
                children: [
                  ...
                  SignUpForm(
                    ...
                    codePayment: codePaymentController,
                    total: totalController,
                    transaksi: transaksiController,
                    expired: expiredController,
                    bank: bankController,
                    cekStatus: () => _cekStatus(context, widget.bank.transaksiID),
                  ),
                ],
              ),
            ),
...
class SignUpForm extends StatelessWidget {
  const SignUpForm({required this.cekStatus, required this.orderId, required this.codePayment, required this.total, required this.transaksi, required this.expired, required this.bank});
  final TextEditingController orderId, codePayment, total, transaksi, expired, bank;
    ...
    return Form(
      child: Column(
        children: [
          ...
          TextFormField(
            readOnly: true,
            controller: codePayment,
            decoration: InputDecoration(
                hintText: "Code Pembayaran VA",
                labelText: "Code VA",
                floatingLabelBehavior: FloatingLabelBehavior.always,
                hintStyle: const TextStyle(color: Color(0xFF757575)),
                contentPadding: const EdgeInsets.symmetric(
