...
class CstorePage extends StatefulWidget {
  CstorePage({required this.cstore});
  Cstore cstore;

  @override
  _CstorePageState createState() => _CstorePageState();
}
...
class _CstorePageState extends State<CstorePage> {
  ...
  void initState() {
    super.initState();
    orderIDController = TextEditingController(text: widget.cstore.orderID);
    codePaymentController = TextEditingController(text: widget.cstore.codePayment.toString());
    totalController = TextEditingController(text: widget.cstore.total);
    transaksiController = TextEditingController(text: formated.format(widget.cstore.transaksi));
    expiredController = TextEditingController(text: formated.format(widget.cstore.expired));
    if(widget.cstore.store == 'alfamart'){
      storeController = TextEditingController(text: "Alfamaret");
    }else if(widget.cstore.store == 'indomaret'){
      storeController = TextEditingController(text: "Indomaret");
    }
  }
              ...
              child: Column(
                children: [
                  ...
                  SignUpForm(
                    ...
                    cekStatus: () => _cekStatus(context, widget.cstore.transaksiID),
                  ),
                ],
              ),
            ),
...
class SignUpForm extends StatelessWidget {
  const SignUpForm({required this.cekStatus, required this.orderId, required this.codePayment, required this.total, required this.transaksi, required this.expired, required this.store});
  final TextEditingController orderId, codePayment, total, transaksi, expired, store;
      ...
      child: Column(
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
                hintText: "Code Pembayaran",
                labelText: "Code Pembayaran",
                floatingLabelBehavior: FloatingLabelBehavior.always,
                hintStyle: const TextStyle(color: Color(0xFF757575)),
                contentPadding: const EdgeInsets.symmetric(
                  horizontal: 24,
