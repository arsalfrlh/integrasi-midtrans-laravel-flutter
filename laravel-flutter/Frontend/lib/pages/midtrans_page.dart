...
import 'package:payment/pages/payment_midtrans_page.dart';
...
class _MidtransPageState extends State<MidtransPage> {
  ...
  void _createPayment(BuildContext context)async{
    if(nameController.text.isNotEmpty && emailController.text.isNotEmpty && totalController.text.isNotEmpty){
      ...
      final response = await midtransService.createPayment(nameController.text, emailController.text, totalController.text);
      ...
      if(response['success'] == true){
        AwesomeDialog(
          ...
          btnOkOnPress: (){
            Navigator.push(context, MaterialPageRoute(builder: (context) => PaymentMidtransPage(snapToken: response['data']['snapToken'])));
          },
        ).show();
      }else{
        AwesomeDialog(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: const Text("SnapToken Payment"),
      ),
      body: SafeArea(
        child: SizedBox(
              ...
              child: Column(
                children: [
                  ...
                  SignUpForm(
                    ...
                    payment: () => _createPayment(context),
                  ),
...
class SignUpForm extends StatelessWidget {
  SignUpForm({required this.name, required this.email, required this.total, required this.payment});
  final TextEditingController name, email, total;
